-- Fix security warnings: Remove SECURITY DEFINER from view and fix function search paths

-- Drop the problematic SECURITY DEFINER view and replace it with a proper view with RLS
DROP VIEW IF EXISTS consultant_companies_secure;

-- Create a proper view without SECURITY DEFINER that relies on RLS
CREATE VIEW public.consultant_companies_secure AS
SELECT 
  id,
  name,
  company,
...
-- Update the function to fix search path warnings
...
-- Update get_consultant_contact_info function to fix search path
